...
function deleteTextNodesRecursive(where) {   
    ...
    for (var i = 0; i < childs.length; i++) {        
        if (childs[i].nodeType == Element.TEXT_NODE) {
            where.removeChild(childs[i]);
            i--;
        } else if (childs[i].nodeType == Element.ELEMENT_NODE) {            
...
function collectDOMStat(root) {
    ...
    for (let child of root.childNodes) {
        iterationNodes(child);    
    }    

    function writeTags(tag, obj) {
        if (tag in obj) {
            obj[tag]++;
        } else {
            obj[tag] = 1;
    ...
    function writeClasses(classList, obj) {
    ...
    function iterationNodes(root) {      
        let tag = root.tagName;
        let classList = root.classList;
        
        if (tag) {
            writeTags(tag, stat.tags);            
        } else {
            stat.texts += 1;
        }
        
        if (classList) {
            writeClasses(classList, stat.classes);            
        }        
